...
RD /S /Q temp
cls
if not exist "dataEU.txt" title 获取CF节点IP&curl --retry 3 https://cfip.pages.dev/EU -o dataEU.txt -#
for /f "tokens=2 delims==" %%a in ('findstr /C:"domain" dataEU.txt') do (
set domain=%%a
)
for /f "tokens=2 delims==" %%a in ('findstr /C:"file" dataEU.txt') do (
set file=%%a
)
for /f "tokens=2 delims==" %%a in ('findstr /C:"database" dataEU.txt') do (
set databaseold=%%a
)
title 生成CF节点IP
set /a i=%random%%%5
set /a n=0
for /f "skip=7" %%a in (dataEU.txt) do (
